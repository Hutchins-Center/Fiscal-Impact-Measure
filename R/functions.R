millions_to_billions <- function(df) {
  df %>%
    dplyr::mutate(dplyr::across(.cols = any_of(
      c('gftfbusx', 'gfeghhx', 'gfeghdx', 'gfeigx')
    ),
    .fns = ~ . / 1000))
}
...
contribution_zero <- function(.data, var) {
  ...
  var_string <-
    rlang::as_string(enexpr(var)) # convert quoted expression to string
  deflator_string <-
    paste0(var_string, "_pi") # create string with the name of relevant deflator
  deflator <-
    rlang::sym(deflator_string) # convert deflator string to symbol
  
  ## Calculate contribution
  .data %>%
    mutate("{{ var }}_cont" := 400 * ({
      {
        var
      }
    } - (1+!!(deflator)) * lag({
...
all_taxes_transfers <- function() {
  ...
  all_taxes_transfers <-
    c(
      glue::glue('{taxes_transfers}'),
      glue::glue('federal_{taxes_transfers}'),
...
fim_plot <- function (df, title, last_date)
{
  df %>% ggplot() + geom_bar(aes(x = date, y = value, fill = variable),
                             stat = "identity",
                             width = 50) + geom_line(aes(x = date,
                                                         y = fiscal_impact_moving_average, colour = "4-quarter moving-average")) +
    geom_point(
      aes(x = date, y = fiscal_impact_moving_average,
    ...
    geom_richtext(
      ...
      "rect",
      xmin = last_date + 40,
      xmax = lubridate::as_date("2022-12-31"),
      ymin = -Inf,
      ymax = Inf,
      alpha = 0.1,
      fill = "yellow"
...
transfers_contributions <- function (df)
{
  ...
  arp_transfers <- c(
    ...
    'health_grants_arp'
  ) %>%  paste0('_minus_neutral_post_mpc')
  all_transfers <-
    c(glue("{transfers}"),
...
cola_adjustment <-  function (df) {
  df %>% mutate(cpiu_g = q_a(cpiu) / 100,
                cola_rate = if_else(month(date) ==
                                      3, lag(cpiu_g, 2), NULL)) %>% fill(cola_rate) %>% mutate(
                                        gftfp_before_cola = gftfp,
                                        health_ui = SMA(yptmd + yptmr + yptu, n = 4),
                                        gftfp_noCOLA = SMA((gftfp -
                                                              health_ui) * (1 - cola_rate), n = 4),
                                        gftfp = gftfp_noCOLA *
                                          (1 + cola_rate) + health_ui,
...
load_budget_projections <- function () {
  readxl::read_xlsx(here::here(
    drake::file_in("data/raw/cbo/cbo_budget_nipas_proj_annual.xlsx")
  )) %>%
    as_tsibble(index = fy)
}
...
load_cbo_projections <- function () {
  ...
  budget %>% annual_to_quarter() %>% mutate(date = lubridate::as_date(date) - 1) %>% dplyr::left_join(economic)
}


load_economic_projections <- function () {
  readxl::read_xlsx(here::here(
    drake::file_in("data/raw/cbo/cbo_econ_proj_quarterly.xlsx")
  )) %>%
...
load_haver_data <- function () {
  ...
  dplyr::left_join(national_accounts, economic_statistics,
                   by = "date") %>% dplyr::mutate(date = lubridate::as_date(date))
}

load_national_accounts <- function () {
  readxl::read_xlsx(here::here(drake::file_in(
    "data/raw/haver/national_accounts.xlsx"
  )))
}

load_economic_statistics <- function () {
  readxl::read_xlsx(here::here(
    drake::file_in("data/raw/haver/economic_statistics.xlsx")
  ))
}
