...
fim[,paste0(tts, "_net")] = lapply(fim[,tts], function(x){
  j = c()
  j[1] = x[1]
  for(i in 2:length(x)){
...
# fim_hist = dplyr::filter(fim, date <= last_hist_date) # some non-dplyr filter() function keeps overriding
...
fim_interactive <- fim %>% 
  ...
  select(year, quarter, fim_bars_ma, recession, fim_bars, state_local_cont, federal_cont, taxes_transfers_cont, projection) %>%
...
thismonth_folder <- as.character(format(as.yearmon(last_q), f = "%m-%Y"))
...
write.csv(fim, paste0(subdir, '/fim-projections-', last_q, ".csv"))
write.csv(fim_interactive, paste0(subdir, '/fim-interactive-', last_q,'.csv'), row.names = F)
write.csv(xx, paste0(subdir, '/xx-', last_q,'.csv'))
