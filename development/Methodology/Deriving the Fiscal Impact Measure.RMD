...
\begin{center} $\sum_{j=1}^{3} \sum_{i=0}^{11} MPC_{j,t-i} T_{j,t-i}$ \end{center}
...
\begin{center} $\tilde{T}_{j,t} = T_{j,t-1}(1+\pi_{C}+ \mu)$ \end{center}
...
\begin{center} $T^C_{t} = \sum_{j=1}^{3} \sum_{i=0}^{11} MPC_{j,t-i} (T_{j,t-i} -\tilde{T}_{j,t-i}) $ \end{center}
...
knitr::kable(tab, "latex", caption = "Assumed MPCs", booktabs = T,col.names = c("t thru t-1","t-2 thru t-3","t-4 thru t-7","t-8 thru t-11","Total MPC")) %>%
