...
pull_data = function(series, database, start.date, frequency = "quarterly"){
  q = haver.data(series, database, eop.dates = T, start = as.Date(start.date, f = "%m-%d-%Y"), frequency = frequency)
...
past = merge(data1, data2, by = "date")
...
forward_aa = aa[aa$date > last_date,]
backward_a = aa[aa$date <= last_date,]
...
forward = which(!(xx$date %in% past$date))
...
# trial for forecasts of state and local tax revenues
forward_alt = which(xx$date >= last_date)
...
write.csv(xx[,c("date", taxpieces, taxpieces_alt)], "taxestest-differenced.csv")
try = xx[,c("date", taxpieces, taxpieces_alt, paste0(taxpieces, "_g"))]
...
fim_past = fim[fim$date >= "2000-01-01" & fim$date <= Sys.Date(),]

# write.csv(fim, paste0('fim-projections-', Sys.Date(), ".csv"))
# write.csv(fim_past, paste0('fim-past-', Sys.Date(), ".csv"))
...
# recessions = data.frame(start = past$date[which(diff(past$Quarterly_NBER_Recession.Expansion__Recession_Shading_..1..1.)==2)], end = past$date[which(diff(past$Quarterly_NBER_Recession.Expansion__Recession_Shading_..1..1.)==-2)][-1])
