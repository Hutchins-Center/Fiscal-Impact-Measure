...
budg = budg %>% mutate(gftfp = gftfp - (yptmd + yptmr + yptu), # temporary
                       cpiu = econ$cpiu[match(budg$date, econ$date)], 
                       cpiu_g = q_a(cpiu)/100, 
                       pcw = hist$pcw[match(budg$date, hist$date)],
                       pcw_g = q_a(pcw)/100)

budg$pcw_g[is.na(budg$pcw_g)] = budg$cpiu_g[is.na(budg$pcw_g)]
budg = budg %>% mutate(cola_rate = ifelse(month(date) == 3, lag(cpiu_g,2), NA))

for(i in 4:nrow(budg)){
  if(is.na(budg$cola_rate[i])){
    budg$cola_rate[i] = budg$cola_rate[i-1] # just forward filling so each of the quarters has its correct cola rate
  }
}

budg = budg %>% mutate(cola_rate_q1 = cola_rate*ifelse(month(date) == 3, 1,0),
                       gftfpCOLA = gftfp) # placeholder for counterfactual series where the only growth is due to colas

for(i in 2:nrow(budg)){
    budg$gftfpCOLA[i] = budg$gftfpCOLA[i-1]*(1+budg$cola_rate_q1[i])
    if(is.na(budg$gftfpCOLA[i])){
      budg$gftfpCOLA[i] = budg$gftfpCOLA[i-1] # this is the counterfactual where SSA only grows with COLAs
    }
}

budg = budg %>% mutate(implied_COLAadj = lag(gftfp,1) - gftfpCOLA,
                       implied_COLAadj = ifelse(implied_COLAadj < 0, 0, implied_COLAadj),
                       gftfp_sspath = gftfp - implied_COLAadj + (yptmd + yptmr + yptu), # add back in medicare, medicaid, ui to raw series
                       smooth_gftfp_sspath = SMA(gftfp_sspath, 4), 
                       gftfp_adj = smooth_gftfp_sspath + implied_COLAadj,
                       gftfp_unadj = gftfp,
                       gftfp = gftfp_adj, # add back in medicare, medicaid, ui to adjusted series and store it as the series to use
                       gftfp_g = q_g(gftfp)) %>% select(-gftfp_adj)

                
# smooth all budget series except social transfers (social security), medicare, medicaid out
