...
output:
  html_document:
    df_print: paged
resource_files:
- results/09-2020/fim-projections-2020-09-12.csv
- results/10-2020/fim-2020-10-15.csv
---
...
both <-
  previous %>%
  mutate(key = 'previous') %>%
  bind_rows(current %>% mutate(key = 'current')) %>%
...
inputPanel(
  ...
  dateRangeInput(
    ...
    start = min(current$date),
    end = max(current$date),
    min = min(current$date),
    max = max(current$date),
    startview = 'year'
  )
)
...
renderPlot({
  bothReactiveMelt() %>%
    ...
    labs(x = '',
         y = '', 
         title = "<span style = 'font-size:16pt; font-family:Helvetica;'>Previous vs Current FIM</span>") +
    theme_hc() +
