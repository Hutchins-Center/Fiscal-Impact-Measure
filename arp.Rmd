...
arp_cont %>% 
  ...
  ggplot(aes(x = date, y = value, fill = name)) +
  geom_col(width = 30) +
  scale_fill_hutchins(labels = c('Aid to small businesses','Unemployment insurance',  'Medicaid expansion', 'Federal discretionary spending^1', 'Other direct aid^2', 'Other aid to vulnerable households^3', 'Rebate Checks' ), pal = 'mixed', rev = TRUE) +
    theme(legend.position = 'top',
  ...
  labs(title = 'Components of the contribution of the American Rescue Plan Act to real GDP growth', 
    ...
    ^1 Federal discretionary spending includes COVID-19 containment and vaccination, aid to state and local governments, and other discretionary spending;<br>
...
arp_cont %>% 
  ...
  labs(title = 'Components of the contribution of the American Rescue Plan Act to real GDP growth', 
       caption = '**Source**: Hutchins Center calculations from Bureau of Economic Analysis and Congressional Budget Office data. 
       **Note:** Other aid to vulnerable households includes food, housing and emergency assistance, ratepayer protection, assistance for older Americans, premium tax credits, and subsidies for COBRA premiums; other direct aid includes the Child Tax Credit, the Earned Income Tax Credit, child care for workers, and dependent care for families.') + 
...
fim %>% 
  select(date, federal_cgrants) %>% 
  filter(date > '2020-09-30' & date < '2021-12-31') %>%
  mutate(date = tsibble::yearquarter(date)) %>% 
  ggplot2::ggplot(aes(x = date, y = federal_cgrants)) +
  ggplot2::geom_line() 
```
