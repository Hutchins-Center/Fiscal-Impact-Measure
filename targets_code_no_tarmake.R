...
current_month <-
  glue::glue(lubridate::month(lubridate::today()),
             '-',
...
override_projections <- function (df) {
  ...
  df %>% left_join(override, by = "date") %>% mutate(
    unemployment_insurance = if_else(
      date >=
        Q3_2020 &
        date <= last_override,
    ...
    federal_unemployment_insurance = if_else(
      date >=
        Q2_2020 &
        date <= last_override,
    ...
    state_unemployment_insurance = if_else(
      date >=
        Q2_2020 &
        date <= last_override,
...
projections =
  ...
  mutate(
    gftfp = gftfp - federal_unemployment_insurance_override - gftfpe,
    gtfp = gstfp + gftfp
  ) %>%
  create_projections() %>%
...
fim =
  ...
  full_join(
    read_xlsx('data/pandemic-legislation/arp_summary.xlsx') %>%
      mutate(date2 = yearquarter(date)) %>% filter(date2 > yearquarter('2021 Q1')),
    by = 'date2'
  ) %>%
  rename(date = date.x) %>%
  as_tibble() %>%
  ...
  mutate(
    rebate_checks = if_else(date == '2021-03-31', rebate_checks - 1348, rebate_checks),
    rebate_checks_arp = if_else(date == '2021-03-31', 1348, rebate_checks_arp),
    federal_subsidies = if_else(date == '2021-03-31', 753, federal_subsidies),
  ...
  mutate(
    rebate_checks_post_mpc = fim::mpc_rebate_checks(rebate_checks_minus_neutral),
    federal_rebate_checks_post_mpc = fim::mpc_rebate_checks(federal_rebate_checks_minus_neutral),
    state_rebate_checks_post_mpc = 0
  ) %>%
  ...
  mutate(non_health_grants_cont = 400 * (
    non_health_grants_post_mpc  - lag(non_health_grants_post_mpc) * (1 + gdppoth + federal_cgrants_pi)
  ) / lag(gdp)) %>%
  mutate(
    federal_grants_cont = federal_grants_cont + non_health_grants_cont,
  ...
  mutate(
    across(
      .cols = all_of(
        c('federal_ui_arp', 'state_ui_arp', 'other_vulnerable') %>% paste0('_minus_neutral')
      ),
    ...
    across(
      .cols = all_of(
        c('rebate_checks_arp', 'other_direct_aid') %>% paste0('_minus_neutral')
      ),
...
fim <-
  fim %>% mutate(rebate_checks = rebate_checks + rebate_checks_arp) %>% mutate(rebate_checks_cont = rebate_checks_cont + rebate_checks_arp_cont)
