...
cbo <-
  ...
  summarise(date  = as_date(date),
        ...
        federal_purchases_deflator = gf / gfh,
        state_purchases_deflator = gs / gsh,
        gdp)  %>% 
  mutate(across(-date,
                ~ .x / lag(.x) - 1,
                .names = "{.col}_growth")) %>% 
  select(date, gdp, ends_with('growth')) %>% 
...
```{r}
for(i in 3:nrow(purchases)){
   purchases[i, 'federal_purchases'] = purchases[i - 1, 'federal_purchases'] * (1 + purchases[i, 'federal_purchases_growth'])
}

for(i in 3:nrow(purchases)){
   purchases[i, 'state_purchases'] = purchases[i - 1, 'state_purchases'] * (1 + purchases[i, 'state_purchases_growth'])
}
purchases %>% 
