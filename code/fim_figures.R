...
fimbars4 = bp(data = fim[,c("date",other_cont, "purchases_cont", "subsidies_cont")], 
              labelz = c(" Health Outlays", " Social Benefits", " Noncorporate Taxes", " Corporate Taxes", " Purchases", " Subsidies"), 
              start.date = "2000-01-01",
              end.date = last_proj_date,
...
if(render){
  rmarkdown::render('Fiscal-Impact-Expanded.RMD', output_file = paste0(subdir, '/Fiscal-Impact-Expanded-', Sys.Date(), '.pdf'), output_format = "pdf_document")
  rmarkdown::render('Fiscal-Impact.RMD', output_file = paste0(subdir, '/Fiscal-Impact-', Sys.Date(), '.pdf'), output_format = "pdf_document")
  
  jpeg(filename = paste0(subdir, '/FIMbars-1-', Sys.Date(), '.jpg'), res = 400, height = 6, width = 8, units = "in")
  fimbars1
  dev.off()
  
  jpeg(filename = paste0(subdir, '/FIMbars-2-', Sys.Date(), '.jpg'), res = 400, height = 6, width = 8, units = "in")
  fimbars2
  dev.off()
