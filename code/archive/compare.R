...
thismonth_folder <- as.character(format(as.yearmon(thismonth), f = "%m-%Y"))
this <- read.csv(paste0(thismonth_folder, "/", daf, thismonth,".csv"), stringsAsFactors = F)
this$projection_date = thismonth_folder

nxtmonth_folder <- as.character(format(as.yearmon(nxtmonth), f = "%m-%Y"))
...
# comps  = c("fim_bars",grep("cont", colnames(mast), value = T), grep("purchases", colnames(mast), value = T))
comps = c("gs", "gf", "yptmd", "yptmr", "gtfp", "gftfp","gstfp", "ysptmd", "yfptmd","gftfpnet", "gstfpnet")
...
pdf(paste0("compare",daf, thismonth_folder, nxtmonth_folder,".pdf"), width = 5, height = 4)
lapply(comps, function(x){
  ggplot(mast, aes_string(x = "date", y = x, fill = "projection_date")) + uni.theme + 
    geom_rect(data = mast, aes(xmin = as.Date(thismonth) + 90, xmax = max(mast$date, na.rm = T), ymin=-Inf, ymax=+Inf), fill = colorz[1], alpha = 0.007) +
    geom_rect(data = mast, aes(xmin = as.Date(nxtmonth) + 90, xmax = max(mast$date, na.rm = T), ymin=-Inf, ymax=+Inf), fill = colorz[2], alpha = 0.01) + 
    # geom_bar() +
