...
fim_interactive <- fim %>% 
  ...
  dplyr::rename("impact" = fim_bars_ma,"total" =  fim_bars, "state_local" = state_local_cont, "federal" =  federal_cont, "consumption" = taxes_transfers_cont)

#calculations

# fim2 <- fim
# fim2$gdppotq <- xx[1:208,"gdppotq"]
# vars <- c("federal_nom","federal_social_benefits","federal_noncorp_taxes", "federal_corporate_taxes","federal_igrants","federal_cgrants")
# fim2[,paste0(vars,"_sg")] <- lapply(vars,function(x) {
#   fim2[,x]/fim2$gdp
# })
# 
# fim2[,paste0(vars,"_sgpot")] <- lapply(vars,function(x) {
#   fim2[,x]/fim2$gdppotq
# })
# 
# fim2 <- fim2[,c("date","gdp","gdppotq",vars,paste0(vars,"_sg"),paste0(vars,"_sgpot"))]
# View(fim2)
# 
# write.csv(fim2,paste0("C:/Users/sbelz/The Brookings Institution/Hutchins Center Team - Documents/Projects/Fiscal Impact",'/fim-shares.csv'))
# write CSVs
...
# fim_old <- read_csv("C:/Users/sbelz/The Brookings Institution/Hutchins Center Team - Documents/Projects/Fiscal Impact/12-2019/fim-projections-2019-12-21.csv")
# fim_old <- fim_old[,-1]
# fim_old[208,] <- NA
# fim_diff <- fim - fim_old
# write.csv(fim_diff, paste0('', '/fim-diffv2', Sys.Date(), ".csv"))
