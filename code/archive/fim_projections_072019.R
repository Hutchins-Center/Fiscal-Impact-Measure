...
## we no longer need to construct our own alternative scenario for federal purchases after 2019. We have it from CBO
          # # construct alternative scenario for federal purchases growth where discretionary spending follows new budget deal, according to https://www.cbo.gov/system/files/2019-07/BipartisanBudgetActof2019.pdf
          # 
          # # Changes on Caps on Spending Subject to Appropriation, by FY
          # outlays = data.frame(
          #   fy = c(2019:2029),
          #   est_outlays = c(0,97926, 130057, 53477, 20785, 12002, 5156, 0, 0, 0, 0)/1000  
          # )
          # 
          ...
          # outlays = rbind(outlays, outlays, outlays, outlays) # we're going to use annual rates anyhow, so just replicate the annual levels over each q
          # outlays = outlays[order(outlays$fy),]
          # # shift budget projection date (FY) to match calendar date
          # outlays$date = shift(econ$date[which(as.integer(format(as.Date(econ$date, format="%d-%m-%Y"),"%Y")) %in% outlays$fy)], 1, type=c("lag")) 
          # 
          # econ$spendingpath = outlays$est_outlays[match(econ$date , outlays$date)]
          # econ$spendingpath[is.na(econ$spendingpath)] = 0
          # spendingpath_fix = max(econ$spendingpath)
          # post_fix = which(econ$date > max(econ$date[econ$spendingpath == spendingpath_fix]))
          # 
          # # alternative f level
          ...
          # econ$gf =  econ$gfb + econ$spendingpath # baslineplus 
          # econ$gf[post_fix] = econ$gf[post_fix] + spendingpath_fix
...
# i don't think this is necessary. It was for one edition of the CBO forecasts, but they look smoother now. 
