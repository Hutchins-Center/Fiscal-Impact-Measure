...
add_factors <- read_excel("documentation/COVID-19 Changes/September/LSFIM_KY_v6_round2_nostimulus.xlsx", 
...
override <- read_excel("documentation/COVID-19 Changes/September/LSFIM_KY_v6_round2_nostimulus.xlsx", 
                       sheet = "FIM Add Factors") %>%
...
saveRDS(fim %>% filter(date > firstDate ), 'data/processed/fim_nostimulus.rds')
...
dir.create(here('results/', thismonth,'no-stimulus'))

# Write csv to current month's folder
results <- 
  list(fim_nostimulus = fim,
       fim_interactive_nostimulus = fim_interactive,
       xx_nostimulus = xx)
