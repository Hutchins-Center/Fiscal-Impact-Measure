...
decompose_totals <- function(df){
  df %>%
    mutate(ui_legislation = 4 * (peuc + pua + puc + other),
           ui_as = total_ui - ui_legislation)
}
ui_allocation <- function(df){
  df %>%
    rename(federal_ui = ui_legislation,
           state_ui = ui_as)
}
...
get_quarterly_ui <- function(){
  read_xlsx('data/pandemic-legislation/pandemic-legislation.xlsx',
    ...
    clean_names() %>%
    rename(peuc = pandemic_emergency_unemployment_compensation,
           pua = pandemic_unemployment_assistance,
           puc = pandemic_unemployment_compensation_payments,
...
get_second_draw  <- function(){
  read_xlsx('data/pandemic-legislation/pandemic-legislation.xlsx',
    ...
    mutate(date = as_date('2021-03-31'),
           yq = yearquarter(date)) 
}
...
ui_override <-
  ...
  mutate(ui_second_draw = coalesce(ui_second_draw, 0),
  ...
  rename(total_ui_override = total_ui,
